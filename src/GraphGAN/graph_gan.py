...
from graph_gan_lstm import dynGanLstm
...
class GraphGAN(object):
    def __init__(self, iteration_num):
        ...
        self.n_node, self.graph = utils.read_edges(
            config.train_filename[iteration_num], config.test_filename)
        self.n_node = 6218
        ...
        if(iteration_num == 0):
            ...
            print("reading previous embeddings results...")
            self.node_embed_init_d = utils.read_embeddings(filename=config.emb_filenames[(iteration_num-1)*2+1],
                                                           n_node=self.n_node,
                                                           n_embed=config.n_emb)
            self.node_embed_init_g = utils.read_embeddings(filename=config.emb_filenames[(iteration_num-1)*2],
                                                           n_node=self.n_node,
                                                           n_embed=config.n_emb)
    ...
    def build_generator(self):
        ...
        with tf.variable_scope("generator"):
            self.generator = generator.Generator(
                n_node=self.n_node, node_emd_init=self.node_embed_init_g)
    ...
    def build_discriminator(self):
        ...
        with tf.variable_scope("discriminator"):
            self.discriminator = discriminator.Discriminator(
                n_node=self.n_node, node_emd_init=self.node_embed_init_d)
    ...
    def train(self, iteration_num):
        ...
        if checkpoint and checkpoint.model_checkpoint_path and config.load_model:
            print("loading the checkpoint: %s" %
                  checkpoint.model_checkpoint_path)
            self.saver.restore(self.sess, checkpoint.model_checkpoint_path)

        self.write_embeddings_to_file(iteration_num)
        ...
        for epoch in range(config.n_epochs):
            ...
            if epoch > 0 and epoch % config.save_steps == 0:
                self.saver.save(self.sess, config.model_log +
                                "model.checkpoint")
    ...
    def prepare_data_for_g(self):
        ...
        for i in self.root_nodes:
            if np.random.rand() < config.update_ratio:
                sample, paths_from_i = self.sample(
                    i, self.trees[i], config.n_sample_gen, for_d=False)
    ...
    def sample(self, root, tree, sample_num, for_d):
        ...
        while len(samples) < sample_num:
            ...
            while True:
                node_neighbor = tree[current_node][1:
                                                   ] if is_root else tree[current_node]
                is_root = False
    ...
    def write_embeddings_to_file(self, iteration_num):
        ...
        for i in range(2):
            ...
            with open(config.emb_filenames[iteration_num*2+i], "w+") as f:
                lines = [str(self.n_node) + "\t" +
                         str(config.n_emb) + "\n"] + embedding_str
                f.writelines(lines)
    ...
    def evaluation(self):
        ...
        if config.app == "link_prediction":
            for i in range(2):
                ...
                result, test_label = lpe.eval_link_prediction()
                results.append(config.modes[i] + ":" + str(result) + "\n")

        with open(config.result_filename, mode="a+") as f:
            f.writelines(results)

        with open(config.test_label_filename, mode="w") as f:
            for j in range(len(test_label)):
                f.write(str(test_label[j]) + "\n")


if __name__ == "__main__":

    for i in range(3):
        graph_gan = GraphGAN(i)
        print("running graphGAN number {}".format(i))
        graph_gan.train(i)
        tf.reset_default_graph()
    dyn_gan_lstm = dynGanLstm()
    dyn_gan_lstm.run()
