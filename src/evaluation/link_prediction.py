...
class LinkPredictEval(object):
    def __init__(self, embed_filename, test_filename, test_neg_filename, n_node, n_embed):
        # each line: node_id, embeddings(dim: n_embed)
        ...
        self.emd = utils.read_embeddings(
            embed_filename, n_node=n_node, n_embed=n_embed)
...
if __name__ == "__main__":
    ...
    lp = LinkPredictEval("snapshotREStrue.emb",
                         test_filename, test_neg_filename, 6218, 25)
    ac, test_label = lp.eval_link_prediction()
    with open("../../data/" + app + "/testLPWithLSTM.txt", mode="w") as f:
